...
OFFER: keep the ORIGINAL offer UI 1:1 — same number of packages/columns, same pack COUNTS (2 / 3 / 6) and supply days. Change only the unit word if our product is not a bottle (jars/tubs/pouches from the mockup). Same button ROLE. Short checkout labels ("buy now", "order", "add to cart") stay that short label (swap brand only if the old brand is IN the button). Never invent a Hormozi/ClickFunnels "GET MY {product}" bundle grid if the source used simple buy-now links (e.g. Digistore). Prices: from product context when known; otherwise keep original numbers and say "keep structure, do not invent numbers".
