 ...
 *
 * IMPORTANT: keep this logic in sync with worker-lib/detect-dynamic-scripts.js
 ...
 */
export function extractInlineScriptText(html: string): string {
...
export function detectDynamicScripts(html: string): DynamicScriptsResult {
  ...
  if (hasContentKw && hasDomMutation && hasTiming) {
