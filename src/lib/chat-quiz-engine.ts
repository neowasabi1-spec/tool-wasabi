/**
 * Landerlab-style messenger quiz (data-next-chat / #chatbox-app).
 *
 * Clone/Swipe strips page JS so affiliate bouncers don't fire in preview.
 * That also kills the inline jQuery that reveals chat bubbles, so these
 * landings look frozen (hero only, buttons never appear). This engine
 * replays the original flow without jQuery or Landerlab's tracker.
 */

const STYLE_ID = 'wasabi-chat-quiz-style';
const SCRIPT_ID = 'wasabi-chat-quiz-engine';

export function isChatQuizHtml(html: string): boolean {
...
/** Editor-only: show every bubble so hidden questions can be edited. */
export function chatQuizEditorRevealCss(): string {
  return (
    `<style data-editor-override id="${STYLE_ID}-editor">` +
    `.nodisplay{display:revert!important}` +
    `.chatbox,.chatbox-message,.chat-btn-block,.quiz-panel{display:block!important}` +
    `.chatbox-message.btns,.chatbox-message.btns-row,.chat-btn-block{display:flex!important}` +
...
export function injectChatQuizEngine(html: string): string {
