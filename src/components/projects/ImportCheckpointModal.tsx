...
export default function ImportCheckpointModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white border border-slate-200 rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col"
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Add the pages of{' '}
                <strong className="text-slate-700">{projectName}</strong> to the
                audit library.
              </p>
            </div>
          </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-2 gap-2">
                        <input
                          ...
                          className="bg-white border border-slate-200 rounded px-2 py-1.5 text-sm text-slate-900 focus:outline-none focus:border-blue-500"
                        />
                        <input
                          type="url"
                          value={row.url}
                          ...
                          placeholder="https://..."
                          className="bg-white border border-slate-200 rounded px-2 py-1.5 text-sm text-slate-900 font-mono focus:outline-none focus:border-blue-500"
                        />
                      </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              disabled={importing}
              className="px-4 py-2 text-sm text-slate-700 hover:text-slate-900 rounded-lg disabled:opacity-50"
            >
              Cancel
            </button>
            <button
...
function SourceBadge({ source }: { source: DetectedUrl['source'] }) {
  ...
  const colorMap = {
    front_end: 'bg-blue-50 text-blue-700 border-blue-200',
    back_end: 'bg-purple-50 text-purple-700 border-purple-200',
